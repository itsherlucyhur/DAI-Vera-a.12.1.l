classdef DAIVeraApp < handle
    ...
    methods
        ...
        function startApp(obj)
            ...
            mlock;
            DAIVeraApp.refcount(obj.Increment);
            ...
            existVal = exist(fullfile(pwd,'DCTFFRGUI')); %#ok<EXIST>
            ...
            if (doesShadowExist && ~pathOne.equals(pathTwo))
                % We are trying to execute the wrong DCTFFRGUI
                errordlg(message('MATLAB:apps:runapp:WrongEntryPoint', 'DCTFFRGUI').getString, ...
            ...
            if nargout(@DCTFFRGUI) == 0
                eval('DCTFFRGUI');
            else
                obj.AppHandle = eval('DCTFFRGUI');
            end

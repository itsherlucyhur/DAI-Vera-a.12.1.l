...
You might have to change paths to select the imported module on Slicer. To do so, Go to: Edit > Application Settings > Modules > Additional module paths > drag and drop the folder workflow (SlicerWorkflowExt-main > DAI_Workflow > workflow). Then navigate to the Module Finder and select "Workflow".  

### Verification
- Check that the DAI Workflow module appears in the module selector 
- Verify that required dependencies (Segment Editor, Extract Centerline, CPR) are available. If not, install any extensions required. 
